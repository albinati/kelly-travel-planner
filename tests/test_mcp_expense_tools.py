...
def test_log_expense_builds_correct_payload(
    ...
    mock_get_user.return_value = SplitwiseUser(
        id=6612239, first_name="Luis", last_name="Albinati", email="luis@example.com"
    )
    mock_get_group.return_value = {
        "id": _DEFAULT_GROUP_ID,
        "members": [{"id": 6612239}, {"id": 42173977}],
    }
    ...
    assert data["group"] == "Family-London2026"
    assert data["paid_by"] == "Luis Albinati"
    # Equal split between 2 members: £12.50 each
    assert data["shares"]["6612239"] == "12.50"
    assert data["shares"]["42173977"] == "12.50"
    ...
    assert kwargs["paid_by_user_id"] == 6612239
...
def test_balances_relativizes_perspective(
    ...
    assert data["group"] == "Family-London2026"
    assert data["me"] == "Luis Albinati"
    # The agent-friendly perspective view: Patricia owes Luis.
    assert data["your_perspective"] == [
        {"direction": "owed_to_you", "from": "Patricia Seda", "amount": "609.57", "currency": "GBP"}
    ]
