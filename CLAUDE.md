...
- **Splitwise** (2): `kelly_log_expense(description, amount, currency="GBP", paid_by_me=True)` and `kelly_expense_balances()` — group is hardcoded to `example-group` (id `0`) and descriptions get auto-prefixed with `[sample-trip]`. Both return `{"error": "..."}` JSON if `SPLITWISE_API_KEY` is absent.
